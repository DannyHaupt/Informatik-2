...
class Player:
    def __init__(self, x, y):
        ...
        self.weapons = [
            ...
            Weapon("RAIL SCOUT", 120, 1.8, 6, 42, 0.004, 1.85, PURPLE, pellets=1, auto=False),
            Weapon("COMBAT KNIFE", 55, 2.2, 999, 0, 0.020, 0.10, WHITE, pellets=1, auto=False),
        ]
        self.damage_flash = 0
        self.hitmarker = 0
        self.shake = 0
...
class Game:
    def __init__(self, mode="offline", network=None, server=None):
        ...
        self.wave = 1
        self.last_hit_id = 0
        self.last_hit_target_id = None
        self.last_hit_damage = 0
        self.last_hit_time = 0
        self.remote_shots_seen = set()
        self.remote_hits_seen = set()
        self.hit_packets = []
    ...
    def update(self, dt):
        ...
        p = self.player
        p.aiming = mouse[2] and p.weapon.name == "RAIL SCOUT"
        for w in p.weapons:
            w.update(dt)
        ...
        # Pickups gibt es jetzt in allen Modi.
        for pickup in self.pickups[:]:
            pickup.t += dt
            if dist(pickup.x, pickup.y, self.player.x, self.player.y) < 48:
                pickup.apply(self.player)
        ...
        if self.mode == "multiplayer" and len(self.pickups) < 5:
            self.spawn_pickups(amount=2)
    ...
    def spawn_pickups(self, amount=6):
    ...
    def ensure_network_hit_state(self):
        if not hasattr(self, "last_hit_id"):
            self.last_hit_id = 0
        if not hasattr(self, "last_hit_target_id"):
            self.last_hit_target_id = None
        if not hasattr(self, "last_hit_damage"):
            self.last_hit_damage = 0
        if not hasattr(self, "last_hit_time"):
            self.last_hit_time = 0
        if not hasattr(self, "hit_packets"):
            self.hit_packets = []
        if not hasattr(self, "remote_hits_seen"):
            self.remote_hits_seen = set()
        if not hasattr(self, "remote_shots_seen"):
            self.remote_shots_seen = set()

    def send_state(self):
        self.ensure_network_hit_state()
        now = time.time()
        ...
        # Hit-Pakete kurz behalten, damit sie sicher beim anderen Client ankommen.
        if not hasattr(self, "hit_packets"):
            self.hit_packets = []

        self.hit_packets = [
            pkt for pkt in self.hit_packets
            if time.time() - pkt.get("time", 0) < 1.2
        ]
        ...
        state = {
            ...
            "shoot_angle": self.last_shoot_angle,
            "shoot_time": self.last_shoot_time,
            "aiming": p.aiming,
            "hit_id": getattr(self, "last_hit_id", 0),
            "hit_target_id": getattr(self, "last_hit_target_id", None),
            "hit_damage": getattr(self, "last_hit_damage", 0),
            "hit_time": getattr(self, "last_hit_time", 0),
            "hit_packets": getattr(self, "hit_packets", []),
        }
    ...
    def handle_remote_shots(self):
        ...
        for rp in self.network.remote_players:
            shooter_id = rp.get("id", 0)

            # Neues System: Liste mehrerer Treffer-Pakete.
            for pkt in rp.get("hit_packets", []):
                pkt_id = pkt.get("id")
                target_id = pkt.get("target_id")
                dmg = pkt.get("damage", 0)
                t = pkt.get("time", 0)

                key = (shooter_id, pkt_id)

                if not pkt_id or key in self.remote_hits_seen:
                    continue

                self.remote_hits_seen.add(key)

                if target_id == my_id and time.time() - t < 1.5:
                    if dmg > 0:
                        self.player.damage(dmg)
                        self.spawn_sparks(self.player.x, self.player.y, RED, 24)
                    continue

            # Altes Einzelpaket zusätzlich als Fallback.
            hid = rp.get("hit_id", 0)
            hit_key = (shooter_id, "single", hid)

            if hid and hit_key not in self.remote_hits_seen:
                self.remote_hits_seen.add(hit_key)

                if rp.get("hit_target_id") == my_id and time.time() - rp.get("hit_time", 0) < 1.0:
                    dmg = rp.get("hit_damage", 0)
                    if dmg > 0:
                        self.player.damage(dmg)
                        self.spawn_sparks(self.player.x, self.player.y, RED, 24)
                    continue

            # Fallback-Raycast beim Empfänger, falls Zielpaket mal verloren geht.
            sid = rp.get("shoot_id", 0)
            shoot_key = (shooter_id, sid)

            if not sid or shoot_key in self.remote_shots_seen:
                continue

            self.remote_shots_seen.add(shoot_key)
            ...
            sx = rp.get("x", 0)
            sy = rp.get("y", 0)
            angle = rp.get("shoot_angle", rp.get("angle", 0))
            d = dist(sx, sy, self.player.x, self.player.y)

            if d > 1500:
                continue

            weapon_id = rp.get("weapon", 0)
            if weapon_id == 4 and d > 95:
                continue

            target_angle = math.atan2(self.player.y - sy, self.player.x - sx)
            diff = abs(angle_diff(target_angle, angle))
            hit_width = math.atan2(52, max(d, 1))

            if diff < hit_width and line_of_sight(sx, sy, self.player.x, self.player.y):
                damages = [16, 34, 11, 120, 55]
                dmg = damages[weapon_id % len(damages)]
                self.player.damage(dmg)
                self.spawn_sparks(self.player.x, self.player.y, RED, 24)
    ...
    def shoot(self, single_click=False):
        ...
        if is_knife:
            hit, _, _ = self.trace_knife(w.damage)
            any_hit = hit
            self.last_shoot_id += 1
            self.last_shoot_angle = p.angle
            self.last_shoot_time = time.time()
        else:
            for pellet in range(w.pellets):
                spread = w.spread * (0.35 if p.aiming else 1.0)
                ...
                hit, _, _ = self.trace_shot(shot_angle, w.damage)
    ...
    def register_multiplayer_hit(self, target_id, damage):
        self.ensure_network_hit_state()
        ...
        self.last_hit_id += 1
        self.last_hit_target_id = target_id
        self.last_hit_damage = int(damage)
        ...
        self.hit_packets.append(pkt)

        # Nicht endlos wachsen lassen
        self.hit_packets = self.hit_packets[-12:]

    def trace_knife(self, damage):
        p = self.player
        reach = 105
        end_x = p.x + math.cos(p.angle) * reach
        end_y = p.y + math.sin(p.angle) * reach
        hit = False
        ...
        if self.mode == "offline":
            ...
            for bot in self.bots:
                ...
                diff = abs(angle_diff(target_ang, p.angle))

                if diff < 1.05 and line_of_sight(p.x, p.y, bot.x, bot.y):
                    score = d + diff * 70
                    if score < best_score:
                        best_score = score
                        best_bot = bot
        ...
        if self.mode == "multiplayer" and self.network:
            ...
            for rp in self.network.remote_players:
                rx = rp.get("x", 0)
                ry = rp.get("y", 0)
                d = dist(p.x, p.y, rx, ry)

                if d > reach:
                    continue

                target_ang = math.atan2(ry - p.y, rx - p.x)
                diff = abs(angle_diff(target_ang, p.angle))

                if diff < 1.05 and line_of_sight(p.x, p.y, rx, ry):
                    score = d + diff * 70
                    if score < best_score:
                        best_score = score
                        best_rp = rp
            ...
            if best_rp:
                p.score += damage
                self.spawn_sparks(best_rp.get("x", 0), best_rp.get("y", 0), RED, 28)
                self.register_multiplayer_hit(best_rp.get("id"), damage)
                hit = True
    ...
    def trace_shot(self, shot_angle, damage):
        ...
        if self.mode == "offline":
            ...
            for bot in self.bots:
                ...
                # bewusst fairere Hitbox, damit Shotgun/Rail zuverlässig treffen
                hit_width = math.atan2(62, max(d, 1))
        ...
        if self.mode == "multiplayer" and self.network:
            ...
            for rp in self.network.remote_players:
                ...
                # größere Hitbox, weil Netzwerkpositionen leicht verzögert sind
                hit_width = math.atan2(68, max(d, 1))
    ...
    def draw_remote_players_3d(self, surf, fov, rays, ray_depths):
        ...
        if self.mode in ("offline", "multiplayer"):
            for pickup in self.pickups:
        ...
        for d, rp, pr in objs:
            ...
            y = int(floor_y - size)
            colors = [CYAN, ORANGE, YELLOW, PURPLE, WHITE]
    ...
    def draw_weapon(self, surf):
        ...
        if w.name == "COMBAT KNIFE":
            cx = WIDTH - 250
            cy = HEIGHT - 210 + kick
            pygame.draw.rect(surf, (185, 140, 100), (cx - 80, cy + 95, 210, 44), border_radius=18)
            pygame.draw.polygon(surf, (205, 215, 225), [(cx + 70, cy + 95), (cx + 245, cy - 35), (cx + 125, cy + 130)])
            pygame.draw.polygon(surf, WHITE, [(cx + 92, cy + 86), (cx + 230, cy - 22), (cx + 132, cy + 116)], 2)
            pygame.draw.rect(surf, (30, 32, 42), (cx + 20, cy + 80, 85, 28), border_radius=8)
            pygame.draw.rect(surf, RED, (cx + 25, cy + 88, 55, 8), border_radius=4)
            return
        ...
        if w.name == "RAIL SCOUT":
            pygame.draw.rect(surf, (13, 16, 24), (cx - 35, cy - 15, 125, 28), border_radius=12)
            pygame.draw.circle(surf, w.color, (cx + 95, cy - 1), 13, 3)
        pygame.draw.circle(surf, w.color, (cx + 265, cy + 65), 8)

    def draw_scope_overlay(self, surf):
        if not self.player.aiming or self.player.weapon.name != "RAIL SCOUT":
            return

        overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 135))
        pygame.draw.circle(overlay, (0, 0, 0, 0), (WIDTH // 2, HEIGHT // 2), 220)
        surf.blit(overlay, (0, 0))

        pygame.draw.circle(surf, (18, 24, 36), (WIDTH // 2, HEIGHT // 2), 224, 8)
        pygame.draw.line(surf, (10, 15, 25), (WIDTH // 2 - 210, HEIGHT // 2), (WIDTH // 2 + 210, HEIGHT // 2), 2)
        pygame.draw.line(surf, (10, 15, 25), (WIDTH // 2, HEIGHT // 2 - 210), (WIDTH // 2, HEIGHT // 2 + 210), 2)
        pygame.draw.circle(surf, CYAN, (WIDTH // 2, HEIGHT // 2), 5, 1)
    ...
    def draw_hud(self, surf):
        ...
        draw_text(surf, "MELEE" if w.name == "COMBAT KNIFE" else f"{w.ammo:02d} / {w.reserve:03d}", FONT_MED, WHITE if w.ammo > 0 else RED, topleft=(WIDTH - 340, HEIGHT - 60))
        if w.reloading:
        ...
        for i, wep in enumerate(p.weapons):
            rect = pygame.Rect(355 + i*124, HEIGHT - 96, 114, 36)
            ...
            draw_text(surf, f"{i+1} {wep.name[:8]}", FONT_TINY, BLACK if active else WHITE, center=rect.center, shadow=False)
    ...
    def draw_crosshair(self, surf):
        if self.player.aiming and self.player.weapon.name == "RAIL SCOUT":
            return
        w = self.player.weapon
...
class Lobby:
    ...
    def render(self):
        ...
        if self.screen == "main":
            ...
            draw_text(WIN, "Weitere mögliche IPs: " + ", ".join(self.all_ips), FONT_TINY, GREY, center=(WIDTH//2, 360))
            self.buttons = self.main_buttons()

        elif self.screen == "host":
            draw_text(WIN, "HOST: eingebauter Server. Noch stabiler: separate Server-Datei nutzen.", FONT, WHITE, center=(WIDTH//2, 260))
        ...
        help_lines = [
            ...
            "Empfohlen: blockstrike_server_tcp.py auf Host starten, dann beide per JOIN GAME verbinden.",
