    ...
    def loop(self):
        while self.running:
            ...
            with self.lock:
                ...
                self.clients[addr]["state"] = msg
                self.clients[addr]["last"] = time.time()
                my_id = self.clients[addr]["id"]

                dead = []
                for a, c in self.clients.items():
                    if time.time() - c["last"] > 6:
                        dead.append(a)
                for a in dead:
                    del self.clients[a]
            ...
            response = {"your_id": my_id, "players": others}
            try:
...
class LanClient:
    ...
    def send(self, state):
        if not self.connected:
            return
        try:
            self.sock.sendto(json.dumps(state).encode("utf-8"), self.server)
        except Exception as e:
            self.error = str(e)
...
class Game:
    ...
    def draw_hud(self, surf):
        ...
        if self.mode == "multiplayer" and self.network:
            status = "connected" if time.time() - self.network.last_recv < 2 else "connecting..."
            draw_text(surf, f"MULTIPLAYER PVP | NET: {status} | deine ID: {self.network.your_id}", FONT_SMALL, CYAN, topleft=(390, HEIGHT - 48))
            draw_text(surf, f"remote players: {len(self.network.remote_players)}", FONT_SMALL, WHITE, topleft=(390, HEIGHT - 24))
...
class Lobby:
    ...
    def render(self):
        ...
        help_lines = [
            ...
            "Host/Join ist PvP gegen eingeloggte Spieler im gleichen Netzwerk.",
            "DHBW-/Uni-/Gast-WLANs blockieren Geräte oft. Dann Handy-Hotspot oder eigenes WLAN nutzen.",
            "Windows/Mac-Firewall: Python bzw. eingehende Verbindungen erlauben.",
        ]
